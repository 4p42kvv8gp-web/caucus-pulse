...
try {
  ...
  if (command === 'status' && !args.length) result = learningStatus(store);
    ...
    result = { runId: report.runId, status: report.status, counts: report.counts, note: report.note };
  } else throw new Error('Use: learning.js status | reserve POST_ID... | create "Session name" POST_ID... | baseline SET_ID | report RUN_ID');
  console.log(JSON.stringify(result, null, 2));
} catch (error) {
  console.error(/^(Invalid |Use:)/.test(error.message) ? error.message : 'Learning operation failed; existing source and reviews are retained.');
  process.exitCode = 1;
} finally { store.close(); }
