...
- [ ] **M3: Connect teaching to future analysis** - Persistent feedback, separate rule proposals, reviewed-example retrieval with holdout exclusions, versioned semantic runs, and rollback are implemented. Next: scheduled reclassification, feedback influence tracking, held-out evaluations, and Jacob's first saved corrections.
