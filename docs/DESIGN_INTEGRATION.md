...
Current implementation follows the supplied neutral system-font visual baseline: 50px shared header, #f5f5f7 ground, white 14px cards without shadows, a two-to-one topic/feed layout, compact source tables, a tracked-case incident timeline, and responsive stacked views. Topics sort by observed members or posts and subtopics open filtered sources. Source counts, recent-language groups and incident suggestions come from the private APIs. No unsupported caucus affiliations, momentum curves, sentiment-quality scores or coordination percentages are shown. The exact screen exports remain unavailable. See [Incident desk](INCIDENT_DESK.md). Browser visual verification is still pending because the existing administrator-policy check was unavailable; no alternate rendering route was used.
