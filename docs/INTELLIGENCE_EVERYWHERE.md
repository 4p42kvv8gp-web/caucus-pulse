...
| Incidents | kind+place string grouping; nightly intel | semantic corroboration (same event, different words/places); provisional single-post cards with evidence spans; audit-driven prompt rules | audit done (docs/INCIDENT_AUDIT.md); corroboration built — `corroborate()` in src/incidents.js, docs/INCIDENT_CORROBORATION.md; provisional build in progress |
