...
6. With the model service stopped, run the Node/Python tests, `node scripts/host-smoke.js` and `node scripts/host-shutdown-smoke.js`. The shutdown command uses isolated synthetic archives to exercise startup cancellation, interrupted inference and subsequent completion after restart. In the configured production environment, run `node scripts/host-preflight.js hosted`. Preserve the reports. `systemd-analyze verify` should validate the prepared unit files on that host, and `cloudflared tunnel ingress validate` should validate the actual tunnel configuration. Resolve failures before enabling the units.
...
9. Enable paid collection only after provider balance/access/pricing are verified, the incomplete List inventory is reconciled, roster evidence is fresh and the chosen cadence has passed bounded recovery checks. Keep collection disabled during the hosting transition. A repeating paid poll service/timer is now prepared but has not been installed or enabled; see [Monitoring setup](MONITORING_SETUP.md). Daily identity renewal and actual schedule-health reporting must be integrated and tested before activation.
...
The authenticated `/api/operations` summary and the Coverage & budget status panel separate model startup, missing services, failed/skipped sources, incomplete indexes, stale account evidence and spending blocks. They make no provider calls and do not report a successful host login, off-host backup or collection schedule. See [Operational status](OPERATIONAL_STATUS.md).
