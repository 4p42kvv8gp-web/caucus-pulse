...
When the store changes, a completed empty, broad-category or explicitly uncertain post is reopened only if a newer relevant source passes the news retriever's reconsideration threshold. Ordinary subtopics such as detention and AI policy remain broad categories for this purpose; an identified event requires a taxonomy row marked `story` or an emerging-event entry. Reviewed corrections remain protected. The previous accepted assignment remains visible while a failed reconsideration stays pending. Successful interpretation stamps the new version, so unchanged or unrelated news does not repeatedly trigger inference. Candidate lists in `data/news/reconsider.json` are diagnostic; the classifier recomputes relevance from the store instead of trusting a potentially stale list. This is lexical retrieval, not a guarantee of coverage or event understanding.
...
## Complete captured repost context

New captures can carry `reposted`, the original post supplied by X with its own ID, author, timestamp and full available wording. The classifier receives this as `reposting` while the member's RT wrapper remains separate. News retrieval also reads the original wording. Requests split on both post/evidence counts and approximately 120,000 input characters so a group of long posts does not silently lose text. This is a size guard, not a tokenizer or exact provider context limit. Previously truncated archive records are not retroactively repaired.
