...
Opposite-position sentences had cosine about 0.90 for MiniLM and 0.95 for BGE, while one paraphrase scored about 0.46/0.77. This directly demonstrates why similarity must not be reported as agreement or coordination. BGE replaced the initial MiniLM default after the separate [subject-group benchmark](SUBJECT_GROUPS.md) recovered the central-bank paraphrase and related detention examples. Both need diverse real-post evaluation before clustering thresholds or model preference are considered validated. The active model is configured in `config/settings.json`; the indexing CLI honors that choice.
