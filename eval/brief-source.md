...
Process **20 items per turn**, in file order. Do not label more than 20 in one
turn, and do not carry reasoning about one batch into the next.
...
4. **A subtopic assignment already counts toward its macro.** If you assign
   `["M05", "M05_S01"]`, do **not** also emit `["M05", null]`. Emit each pair
...
6. **`labels: []` has two distinct meanings and you must tell them apart.**
   ...
   **`[macro, null]` is NOT a gap.** A gap means *no macro fits at all*. Worked
   examples: a post about a new rural broadband grant is `[["M14", null]]`,
...
9. **Include every `item_id` from the batch exactly once**, in order. Do not
...
- A member's condolence post about a shooting in her city **IS** a topic:
  `[["M12", "M12_S01"]]` (guns & public safety / gun violence). A member
