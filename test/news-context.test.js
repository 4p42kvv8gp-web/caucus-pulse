...
test('injection text stays inside the evidence block as quoted data; the frame says so; the compact line is capped', async () => {
  ...
  for (const key of ['id', 'publisher', 'date', 'kind', 'url', 'text', 'publishedAt', 'fetchedAt', 'version', 'publishedAfterPost', 'truncated']) assert.ok(key in line);
...
test('evidenceForPosts is bounded per post and per chunk and reports the store version', async () => {
  ...
  assert.equal(Object.values(capped.byPost).flat().length, 1); // a hard chunk cap also bounds the final post
