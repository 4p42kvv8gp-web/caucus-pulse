...
test('classifier migration and unfinished jobs survive reopening without requiring a local model', () => {
  ...
  try {
    add(store);
    store.db.exec('DROP TRIGGER classifier_post_insert; DROP TRIGGER classifier_post_update; DROP TABLE classifier_jobs; DROP TABLE classifier_profiles; UPDATE schema_version SET version=7;');
